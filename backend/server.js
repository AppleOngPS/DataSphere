...
app.use(async (req, res, next) => {
  if (req.user && req.user.publicMetadata && req.user.publicMetadata.role) {
    ...
    try {
      ...
      res.status(500).json({ message: "Error storing user data", error: err.message });
    }
  } else {
...
app.post('/webhooks/clerk', express.raw({ type: 'application/json' }), webhookController.handleWebhook);
...
app.get("/bookings/child/:childID", bookingController.getAllBookingsForChild);
